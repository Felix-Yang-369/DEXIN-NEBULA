...
德小馨当前基线通过 DeepSeek Chat Completions API 提供回答能力。所有请求只从
...
在获准的部署平台秘密管理中添加相同变量。开发、预览和生产环境应使用相互隔离的
Key，并为生产账户设置预算与告警。

## 4. 当前数据范围

当前版本可向 DeepSeek 发送与问题匹配且当前员工有权查看的有限记录：
...
不会发送未命中的广泛数据、未授权字段、文件二进制正文或其他员工的对话。文件来源
若仅包含元数据，回答不得声称已阅读正文。模型只负责整理检索结果，Supabase RLS、
字段限制和服务端检索共同负责权限与数据最小化。
