...
export function JournalEntryForm({ accounts, bookId, today }: { accounts: AccountOption[]; bookId: string; today: string }) {
  ...
  return <form action={createAccountingJournalAction} className="mt-5 space-y-4">
    ...
    <div className="grid gap-3 md:grid-cols-[180px_1fr_130px]">
      <label className="text-xs text-muted-foreground">凭证日期<input className="mt-1 h-10 w-full rounded-md border border-border px-3 text-xs text-foreground" defaultValue={today} name="entryDate" required type="date" /></label>
      <label className="text-xs text-muted-foreground">凭证摘要<input className="mt-1 h-10 w-full rounded-md border border-border px-3 text-xs text-foreground" name="summary" placeholder="说明本笔经济业务" required /></label>
      <label className="text-xs text-muted-foreground">附件张数<input className="mt-1 h-10 w-full rounded-md border border-border px-3 text-xs text-foreground" defaultValue="0" min="0" name="attachmentCount" type="number" /></label>
    </div>
    <div className="overflow-x-auto rounded-md border border-border"><table className="w-full min-w-[780px] text-left text-xs"><thead className="bg-muted text-muted-foreground"><tr><th className="p-3">科目</th><th className="p-3">分录摘要</th><th className="p-3 text-right">借方</th><th className="p-3 text-right">贷方</th><th className="w-12 p-3" /></tr></thead><tbody>
    ...
    </tbody><tfoot><tr className="border-t border-border bg-muted font-medium"><td className="p-3" colSpan={2}>合计 · {balanced ? "借贷平衡" : "借贷未平衡"}</td><td className="p-3 text-right tabular-nums">¥{totals.debit.toFixed(2)}</td><td className="p-3 text-right tabular-nums">¥{totals.credit.toFixed(2)}</td><td /></tr></tfoot></table></div>
    <div className="flex flex-wrap items-center justify-between gap-3"><button className="inline-flex h-9 items-center gap-2 rounded-md border border-border px-4 text-xs" onClick={() => { setLines((current) => [...current, { key: nextKey, account_id: accounts[0]?.id ?? "", summary: "", debit_amount: "", credit_amount: "" }]); setNextKey((value) => value + 1); }} type="button"><Plus className="size-3.5" />增加分录</button><button className="h-10 rounded-md bg-primary px-5 text-xs text-primary-foreground disabled:cursor-not-allowed disabled:opacity-40" disabled={!balanced || lines.some((line) => !line.account_id || !line.summary)} type="submit">保存凭证草稿</button></div>
  </form>;
}
