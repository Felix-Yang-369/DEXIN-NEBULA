...
export function OpeningBalanceForm({ accounts, bookId, fiscalYear }: { accounts: Account[]; bookId: string; fiscalYear: number }) {
  ...
  return <form action={createOpeningBalanceAction} className="mt-5">
    ...
    <div className="overflow-x-auto rounded-xl border border-border"><table className="w-full min-w-[680px] text-left text-[10px]"><thead className="bg-[#f5f8fa] text-muted-foreground"><tr><th className="p-3">科目</th><th className="p-3 text-right">期初借方</th><th className="p-3 text-right">期初贷方</th><th className="w-12 p-3" /></tr></thead><tbody>
    ...
    </tbody><tfoot><tr className="border-t border-border bg-[#fafcfe] font-medium"><td className="p-3">合计 · {balanced ? "借贷平衡" : "借贷未平衡"}</td><td className="p-3 text-right tabular-nums">¥{totals.debit.toFixed(2)}</td><td className="p-3 text-right tabular-nums">¥{totals.credit.toFixed(2)}</td><td /></tr></tfoot></table></div>
    <div className="mt-4 flex flex-wrap items-center justify-between gap-3"><button className="inline-flex h-9 items-center gap-2 rounded-xl border border-border px-4 text-xs" onClick={() => { setLines((current) => [...current, { key: nextKey, account_id: accounts[0]?.id ?? "", debit_amount: "", credit_amount: "" }]); setNextKey((value) => value + 1); }} type="button"><Plus className="size-3.5" />增加科目</button><button className="h-10 rounded-xl bg-primary px-5 text-xs text-primary-foreground disabled:opacity-40" disabled={!balanced || lines.some((line) => !line.account_id || (Number(line.debit_amount || 0) > 0) === (Number(line.credit_amount || 0) > 0))} type="submit">生成期初凭证草稿</button></div>
  </form>;
}
