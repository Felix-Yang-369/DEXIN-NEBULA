    ...
    <Card className="min-w-0 overflow-hidden bg-card transition duration-200  hover:border-border ">
      ...
      <CardContent className="pt-4">
        <div className="mb-4 flex items-end gap-4">
          <div>
            <div className="text-xs text-foreground">销售额（元）</div>
          ...
          <div className="pb-0.5 text-xs font-medium text-foreground">权限内实时汇总</div>
        </div>
          ...
          {data.length > 0 ? (
            <ResponsiveContainer height="100%" width="100%">
              ...
              >
                ...
                <Line
                  dataKey="orders"
                  dot={{ fill: "var(--muted-foreground)", r: 2.5, strokeWidth: 0 }}
                  name="订单数"
                  stroke="var(--border)"
                  strokeWidth={2}
                  type="monotone"
            ...
            <div className="grid h-full place-items-center text-xs text-foreground">
              暂无销售趋势数据
            </div>
          )}
        </div>
        <div className="mt-2 flex justify-center gap-6 text-xs text-foreground">
          <span className="inline-flex items-center gap-2">
            <i className="size-2.5 rounded-full bg-primary" />
            销售额
          </span>
          <span className="inline-flex items-center gap-2">
            <i className="size-2.5 rounded-full bg-muted" />
            订单数
          </span>
        </div>
      </CardContent>
