  ...
  return (
    ...
    >
      <main className="mx-auto max-w-[1440px] p-4 sm:p-6 xl:p-8">
        ...
        <section className="mt-4 rounded-md bg-primary px-6 py-8 text-white sm:px-8">
        ...
        <section className="mt-5 grid gap-4 sm:grid-cols-3">
          {[
            ...
            return (
              <article className="rounded-md border border-border bg-white p-5" key={String(label)}>
        ...
        <div className="mt-5 grid items-start gap-5 xl:grid-cols-2">
          <section className="rounded-md border border-border bg-white p-5 sm:p-6">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-muted-foreground">如 P1 / P2 / P3</span>
            </div>
            <div className="mt-4 space-y-2">
              {levels.map((level) => (
                <div className="flex items-center justify-between rounded-md bg-muted p-3" key={level.id}>
                  <div>
                    <span className="text-xs font-semibold">{level.code}</span>
                    <span className="ml-2 text-xs">{level.name}</span>
                  </div>
                  <span className="text-xs text-muted-foreground">序列 {level.rank}</span>
                </div>
              ))}
              {!levels.length && <div className="rounded-md bg-muted p-4 text-xs text-foreground">尚未配置职级，不自动创建未经确认的等级定义。</div>}
            </div>
            {canManage && (
              <form action={saveJobLevelAction} className="mt-4 grid gap-3 sm:grid-cols-3">
                <input className="h-10 rounded-md border border-border px-3 text-xs" name="code" placeholder="代码，如 P1" required />
                <input className="h-10 rounded-md border border-border px-3 text-xs" name="name" placeholder="职级名称" required />
                <input className="h-10 rounded-md border border-border px-3 text-xs" min="1" name="rank" placeholder="排序" required type="number" />
                <input className="h-10 rounded-md border border-border px-3 text-xs sm:col-span-2" name="description" placeholder="职级说明（选填）" />
                <button className="h-10 rounded-md bg-primary text-xs text-primary-foreground" type="submit">新增职级</button>
              </form>
            )}
          </section>
          ...
          <section className="rounded-md border border-border bg-white p-5 sm:p-6">
            <div className="flex items-center justify-between">
              ...
              <Link className="text-xs text-primary" href="/organization">查看组织架构图</Link>
            </div>
            <div className="mt-4 space-y-2">
              {positions.map((position) => (
                <div className="rounded-md bg-muted p-3" key={position.id}>
                  <div className="flex items-center justify-between">
              ...
              {!positions.length && <div className="rounded-md bg-muted p-4 text-xs text-foreground">尚未配置岗位。</div>}
            </div>
            {canManage && (
              <form action={savePositionAction} className="mt-4 grid gap-3 sm:grid-cols-2">
                <input className="h-10 rounded-md border border-border px-3 text-xs" name="code" placeholder="岗位代码" required />
                <input className="h-10 rounded-md border border-border px-3 text-xs" name="name" placeholder="岗位名称" required />
                <select className="h-10 rounded-md border border-border px-3 text-xs" name="departmentId">
                  <option value="">暂不分配部门</option>
                  {departments.map((department) => (
                    <option key={department.id} value={department.id}>{department.name}</option>
                  ))}
                </select>
                <select className="h-10 rounded-md border border-border px-3 text-xs" name="jobLevelId">
                  <option value="">暂不设置职级</option>
                  {levels.map((level) => (
                    <option key={level.id} value={level.id}>{level.code} · {level.name}</option>
                  ))}
                </select>
                <input className="h-10 rounded-md border border-border px-3 text-xs" min="0" name="headcount" placeholder="岗位编制" type="number" />
                <input className="h-10 rounded-md border border-border px-3 text-xs" name="responsibilities" placeholder="岗位职责摘要" />
                <button className="h-10 rounded-md bg-primary text-xs text-primary-foreground sm:col-span-2" type="submit">新增岗位</button>
              </form>
            )}
          </section>
        </div>
