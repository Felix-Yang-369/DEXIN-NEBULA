      ...
      <main className="mx-auto max-w-[1440px] p-4 sm:p-6 xl:p-8">
        ...
        <section className="relative mt-5 overflow-hidden rounded-md bg-primary px-6 py-8 text-white sm:px-8">
          ...
          <div className="relative flex max-w-4xl flex-col justify-between gap-6 md:flex-row md:items-end">
            ...
            <Link
              className="inline-flex h-10 shrink-0 items-center gap-2 rounded-md bg-muted px-4 text-xs font-medium text-foreground"
              href="/requests/leave"
            >
            ...
            {currentBalance ? (
              <dl className="mt-5 grid grid-cols-2 gap-3">
                {[
                  ...
                  <div className="rounded-md bg-muted p-4" key={String(label)}>
              ...
              <div className="mt-5 rounded-md border border-dashed border-border p-8 text-center text-xs text-muted-foreground">
                本年度假期账户尚未配置，请联系行政人事。
              </div>
            )}
          </article>
            ...
            {typeTotals.length ? (
              <div className="mt-6 space-y-4">
                {typeTotals.map(([type, days]) => (
                  <div
                    className="grid grid-cols-[72px_1fr_42px] items-center gap-3 text-xs"
                    key={type}
                  >
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-2">
          <div className="rounded-md border border-border bg-muted px-5 py-4 text-xs leading-5 text-muted-foreground">
            <strong className="text-foreground">同步规则：</strong>
            福利假计入年假已用，病假计入病假已用；事假、婚假、丧假、产假等仅记录考勤，不扣减年假。余额未配置或不足时保留申请结果并提示人事处理。
          </div>
          <div className="rounded-md border border-border bg-white px-5 py-4 text-xs leading-5 text-muted-foreground">
