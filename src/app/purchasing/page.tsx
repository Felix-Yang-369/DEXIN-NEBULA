...
const requestStatus: Record<string, [string, string]> = {
  submitted: ["待审批", "bg-muted text-foreground"],
  approved: ["已通过", "bg-muted text-foreground"],
  rejected: ["已驳回", "bg-muted text-foreground"],
  converted: ["已转订单", "bg-muted text-foreground"],
  cancelled: ["已撤回", "bg-muted text-muted-foreground"],
};

const orderStatus: Record<string, [string, string]> = {
  draft: ["草稿", "bg-muted text-muted-foreground"],
  confirmed: ["待到货", "bg-muted text-foreground"],
  partial_received: ["部分到货", "bg-muted text-foreground"],
  received: ["已入库", "bg-muted text-foreground"],
  cancelled: ["已取消", "bg-muted text-foreground"],
};
  ...
  return (
    ...
    >
      <main className="mx-auto max-w-[1440px] p-4 sm:p-6 xl:p-8">
        <section id="overview" className="ui-page-header">
          ...
          <div className="relative max-w-3xl">
            <div className="text-xs tracking-[0.2em] text-muted-foreground">V0.9 · PROCUREMENT & FINANCE LOOP</div>
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              <a className="rounded-md bg-white px-4 py-2 text-xs font-semibold text-foreground" href="#requests">发起采购申请</a>
              <Link className="rounded-md border border-white/25 px-4 py-2 text-xs font-semibold" href="/purchasing/control">询价 · 质检 · 三单匹配</Link>
              <Link className="rounded-md border border-white/25 px-4 py-2 text-xs font-semibold" href="/finance/bank-reconciliation">银行核销 <ArrowRight className="ml-1 inline size-3" /></Link>
            </div>
          </div>
        </section>
        ...
        {migrationMissing && <div className="mt-4 rounded-md border border-border bg-muted px-4 py-3 text-xs text-foreground">V0.9 数据库迁移尚未执行。页面结构已就绪，执行新迁移后将切换为真实采购数据。</div>}
        ...
        <section className="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
          {[
            ...
            <div className="rounded-md border border-border/70 bg-white p-4 " key={label}>
              <div className="flex items-center justify-between"><span className="text-xs text-muted-foreground">{label}</span><Icon className="size-4 text-foreground" /></div>
        ...
        <section id="requests" className="mt-6 grid gap-5 xl:grid-cols-[0.9fr_1.1fr]">
          <div className="rounded-md border border-border/70 bg-white p-5">
            <div className="mb-4"><div className="text-sm font-semibold text-foreground">发起采购申请</div><div className="mt-1 text-xs text-muted-foreground">所有在职员工均可提交，采购负责人审批后转采购订单。</div></div>
            <PurchaseRequestBuilder products={products} />
          </div>
          <div className="rounded-md border border-border/70 bg-white p-5">
            <div className="mb-4 flex items-center justify-between"><div><div className="text-sm font-semibold text-foreground">采购申请</div><div className="mt-1 text-xs text-muted-foreground">我的申请与权限内待审批申请</div></div><span className="text-xs text-muted-foreground">{requests.length} 条</span></div>
            <div className="space-y-2">
              {requests.length ? requests.map((request) => {
                const status = requestStatus[request.status] ?? [request.status, "bg-muted text-muted-foreground"];
                return <div className="rounded-lg border border-border/65 p-4" key={request.id}>
                  <div className="flex flex-wrap items-start justify-between gap-3"><div><div className="text-xs font-semibold text-foreground">{request.title}</div><div className="mt-1 text-xs text-muted-foreground">{request.request_no} · {one(request.requester)?.name ?? "员工"} · 需用 {date(request.required_on)}</div></div><span className={`rounded-lg px-2 py-1 text-xs ${status[1]}`}>{status[0]}</span></div>
                  <div className="mt-3 text-xs text-muted-foreground">{request.items.map((item) => `${item.product_name} × ${item.quantity}`).join("；") || "暂无商品明细"}</div>
                  <div className="mt-3 flex flex-wrap items-center justify-between gap-2"><span className="text-xs font-semibold text-foreground">{Number(request.estimated_amount) > 0 ? `预计 ${money(request.estimated_amount)}` : "待采购核价"}</span><div className="flex gap-2">
                    {request.status === "submitted" && canManage && <><form action={transitionPurchaseRequestAction}><input name="requestId" type="hidden" value={request.id} /><input name="targetStatus" type="hidden" value="approved" /><button className="rounded-lg bg-muted px-3 py-1.5 text-xs font-medium text-foreground" type="submit">通过</button></form><form action={transitionPurchaseRequestAction} className="flex gap-1"><input name="requestId" type="hidden" value={request.id} /><input name="targetStatus" type="hidden" value="rejected" /><input className="w-28 rounded-lg border border-border px-2 text-xs" name="note" placeholder="驳回原因" required /><button className="rounded-lg bg-muted px-3 py-1.5 text-xs text-foreground" type="submit">驳回</button></form></>}
                    {request.status === "submitted" && request.requester_employee_id === employee.id && <form action={transitionPurchaseRequestAction}><input name="requestId" type="hidden" value={request.id} /><input name="targetStatus" type="hidden" value="cancelled" /><input name="note" type="hidden" value="申请人撤回" /><button className="rounded-lg border border-border px-3 py-1.5 text-xs text-muted-foreground" type="submit">撤回</button></form>}
                  </div></div>
                </div>;
              }) : <div className="rounded-lg bg-muted px-4 py-10 text-center text-xs text-muted-foreground">暂无采购申请</div>}
            </div>
          </div>
        </section>

        {canManage && <section id="orders" className="mt-6 rounded-md border border-border/70 bg-white p-5"><div className="mb-4"><div className="text-sm font-semibold text-foreground">创建采购订单</div><div className="mt-1 text-xs text-muted-foreground">采购价默认取产品中心有效采购价，仍可按本次合同议价调整。</div></div><PurchaseOrderBuilder approvedRequests={approvedRequests.map((item) => ({ id: item.id, requestNo: item.request_no, title: item.title }))} products={products} suppliers={(suppliersResult.data ?? []).map((item) => ({ id: item.id, name: item.name, supplierNo: item.supplier_no }))} warehouses={(warehousesResult.data ?? []).map((item) => ({ id: item.id, name: item.name, code: item.code }))} /></section>}

        <section className="mt-6 rounded-md border border-border/70 bg-white p-5">
          <div className="mb-4 flex items-center justify-between"><div><div className="text-sm font-semibold text-foreground">采购订单执行</div><div className="mt-1 text-xs text-muted-foreground">确认订单后由仓储按实际到货办理分批入库。</div></div><span className="text-xs text-muted-foreground">{orders.length} 单</span></div>
          <div className="overflow-x-auto"><table className="w-full min-w-[900px] text-left text-xs"><thead><tr className="border-b border-border text-xs text-muted-foreground"><th className="pb-3 font-medium">采购订单</th><th className="pb-3 font-medium">供应商 / 仓库</th><th className="pb-3 font-medium">日期</th><th className="pb-3 font-medium">履约</th><th className="pb-3 text-right font-medium">金额</th><th className="pb-3 text-right font-medium">操作</th></tr></thead><tbody>
            {orders.map((order) => { const status = orderStatus[order.status] ?? [order.status, "bg-muted"]; const ordered = order.items.reduce((sum, item) => sum + Number(item.quantity), 0); const received = order.items.reduce((sum, item) => sum + Number(item.received_quantity), 0); return <tr className="border-b border-border/60 last:border-0" key={order.id}><td className="py-3"><div className="font-medium text-foreground">{order.order_no}</div><div className="mt-1 text-xs text-muted-foreground">采购：{one(order.buyer)?.name ?? "未指定"}</div></td><td className="py-3"><div>{one(order.supplier)?.name ?? "供应商"}</div><div className="mt-1 text-xs text-muted-foreground">入库：{one(order.warehouse)?.name ?? "仓库"}</div></td><td className="py-3"><div>{date(order.order_date)}</div><div className="mt-1 text-xs text-muted-foreground">预计 {date(order.expected_arrival_on)}</div></td><td className="py-3"><span className={`rounded-lg px-2 py-1 text-xs ${status[1]}`}>{status[0]}</span><div className="mt-2 text-xs text-muted-foreground">已收 {received} / {ordered}</div></td><td className="py-3 text-right font-semibold">{money(order.total_amount)}</td><td className="py-3"><div className="flex justify-end gap-2">{order.status === "draft" && canManage && <><form action={transitionPurchaseOrderAction}><input name="orderId" type="hidden" value={order.id} /><input name="targetStatus" type="hidden" value="confirmed" /><button className="rounded-lg bg-muted px-3 py-1.5 text-xs text-foreground" type="submit">确认</button></form><form action={transitionPurchaseOrderAction}><input name="orderId" type="hidden" value={order.id} /><input name="targetStatus" type="hidden" value="cancelled" /><input name="note" type="hidden" value="采购负责人取消" /><button className="rounded-lg bg-muted px-3 py-1.5 text-xs text-foreground" type="submit">取消</button></form></>}</div></td></tr>; })}
        ...
        <section id="receiving" className="mt-6 grid gap-5 xl:grid-cols-[1.1fr_0.9fr]">
          <div><div className="mb-3"><div className="text-sm font-semibold text-foreground">待到货入库</div><div className="mt-1 text-xs text-muted-foreground">过账时同步库存批次、入库流水和采购应付。</div></div><div className="space-y-3">{canReceive && receivingOrders.length ? receivingOrders.map((order) => <GoodsReceiptForm key={order.id} orderId={order.id} orderNo={order.order_no} lines={order.items.filter((item) => Number(item.received_quantity) < Number(item.quantity)).map((item) => ({ purchaseOrderItemId: item.id, productName: item.product_name, orderedQuantity: Number(item.quantity), receivedQuantity: Number(item.received_quantity), quantity: String(Number(item.quantity) - Number(item.received_quantity)), productionDate: "", shelfLifeMonths: "" }))} />) : <div className="rounded-md border border-border/70 bg-white px-5 py-12 text-center text-xs text-muted-foreground">{canReceive ? "暂无待到货订单" : "仅仓储人员可办理到货入库"}</div>}</div></div>
          <div className="rounded-md border border-border/70 bg-white p-5"><div className="mb-4"><div className="text-sm font-semibold text-foreground">最近入库单</div><div className="mt-1 text-xs text-muted-foreground">每张入库单对应一笔实际采购应付。</div></div><div className="space-y-2">{receipts.length ? receipts.map((receipt) => <div className="flex items-center justify-between rounded-md bg-muted px-3 py-3" key={receipt.id}><div><div className="text-xs font-medium">{receipt.receipt_no}</div><div className="mt-1 text-xs text-muted-foreground">{one(receipt.supplier)?.name} · {one(receipt.warehouse)?.name} · {date(receipt.received_on)}</div></div><div className="text-xs font-semibold text-foreground">{money(receipt.total_amount)}</div></div>) : <div className="py-10 text-center text-xs text-muted-foreground">暂无入库单</div>}</div>{canFinance && <Link className="mt-4 inline-flex items-center gap-1 text-xs font-medium text-foreground" href="/finance?book=payable#documents">查看采购应付 <ArrowRight className="size-3" /></Link>}</div>
        </section>
      </main>
    </WorkflowShell>
  );
