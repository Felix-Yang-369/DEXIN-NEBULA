          ...
          <div className="relative flex flex-col gap-5 sm:flex-row sm:items-center">
            <div className="flex shrink-0 flex-col items-center gap-2.5">
              ...
              <form action={uploadOwnAvatarAction} className="flex flex-col items-center gap-1.5">
                <label className="group flex cursor-pointer items-center gap-1.5 rounded-full border border-white/18 bg-white/8 px-3 py-1.5 text-xs text-white/72 transition hover:border-border hover:bg-white/12 hover:text-white">
                ...
                <button className="text-xs text-muted-foreground underline decoration-primary/30 underline-offset-2 hover:text-white" type="submit">
                  确认上传
                </button>
              </form>
            </div>
            ...
            <span className="w-fit rounded-full border border-border bg-muted px-3 py-1.5 text-xs text-muted-foreground sm:ml-auto">
              账号状态正常
            </span>
          </div>
        </section>
            ...
            <section className="rounded-md border border-border/75 bg-white p-5 sm:p-6">
              ...
              <div className="mt-5 grid gap-3 sm:grid-cols-2">
                {[
                  ...
                  <div className="rounded-lg border border-border/70 bg-muted p-4" key={String(label)}>
              ...
              <div className="mt-4 rounded-lg bg-muted px-4 py-3 text-xs leading-5 text-muted-foreground">
            ...
            <section className="rounded-md border border-border/75 bg-white p-5 sm:p-6">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {currentEmployee.roleCodes.map((code) => (
                  <span className="rounded-full border border-border bg-muted px-3 py-1.5 text-xs font-medium text-foreground" key={code}>
          ...
          <aside className="space-y-5">
            ...
            <section className="rounded-md border border-border/75 bg-white p-5">
              ...
              <div className="mt-4 grid gap-2">
                <Link className="flex h-10 items-center justify-center rounded-md border border-primary/20 bg-primary/[0.04] px-4 text-xs font-medium text-primary hover:bg-primary/[0.08]" href="/account/password">
                  修改登录密码
                </Link>
                <Link className="flex h-10 items-center justify-center rounded-md bg-primary px-4 text-xs font-medium text-primary-foreground" href={`/employees/${currentEmployee.id}`}>
                  查看我的完整员工档案
                </Link>
                <Link className="flex h-10 items-center justify-center rounded-md border border-border bg-white px-4 text-xs font-medium text-muted-foreground hover:bg-muted" href="/help">
                  获取账号使用帮助
                </Link>
              </div>
            </section>
