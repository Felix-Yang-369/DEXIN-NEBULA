    ...
    >
      <main className="mx-auto max-w-[1440px] p-4 sm:p-6 xl:p-8">
        <section className="ui-page-header">
          ...
          <div className="relative flex flex-col justify-between gap-6 lg:flex-row lg:items-end">
            ...
            <div className="flex gap-2">
              <Link className="rounded-md border border-white/15 bg-white/10 px-4 py-3 text-xs backdrop-blur" href="/customers">
                客户档案
              </Link>
              <Link className="rounded-md bg-muted px-4 py-3 text-xs font-semibold text-foreground" href="/quotes">
                报价中心
              </Link>
            </div>
          </div>
        ...
        {(feedback.created || feedback.updated) && (
          <div className="mt-5 rounded-lg border border-border bg-muted px-4 py-3 text-xs text-primary">
            {feedback.created || feedback.updated}
          </div>
        )}
        {feedback.error && (
        ...
        <section className="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          {[
            ...
            return (
              <article className="rounded-md border border-white bg-white/82 p-5 " key={String(label)}>
                ...
                <div className="mt-4 border-t border-border/70 pt-3 text-xs text-muted-foreground">{String(note)}</div>
              </article>
            );
          })}
        </section>
        ...
        <div className="mt-5 grid items-start gap-5 xl:grid-cols-[minmax(0,1.35fr)_minmax(380px,.65fr)]">
          <div className="space-y-5">
            <section className="scroll-mt-24 overflow-hidden rounded-md border border-border/80 bg-white" id="orders">
              <div className="flex items-center justify-between border-b border-border/70 px-5 py-5 sm:px-6">
                <div>
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">订单、客户法律实体和商品交易快照</p>
                </div>
                        ...
                        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                          <div className="min-w-0">
                            <div className="flex flex-wrap items-center gap-2">
                              <span className="font-mono text-xs text-foreground">{order.order_no}</span>
                              <span className={`rounded-full px-2.5 py-1 text-xs ${statusToneClass(order.status)}`}>{orderStatusLabels[order.status]}</span>
                              {!order.legal_entity_id && <span className="rounded-full bg-amber-50 px-2.5 py-1 text-xs text-amber-700">待绑定法律实体</span>}
                            </div>
                              ...
                              <div className="flex flex-wrap gap-2">
                                {availableSalesOrderTransitions(order.status).includes("confirmed") && (
                                  <form action={transitionSalesOrderAction}>
                                    ...
                                    <button className="rounded-lg bg-primary px-3 py-2 text-xs font-medium text-white" type="submit">提交确认审批</button>
                                  </form>
                                )}
                                {availableSalesOrderTransitions(order.status).includes("cancelled") && <form action={transitionSalesOrderAction} className="flex gap-1.5">
                                  ...
                                  <input className="h-8 w-32 rounded-lg border border-border px-2 text-xs" name="note" placeholder="取消原因" required />
                                  <button className="rounded-lg border border-red-200 px-3 text-xs text-red-600" type="submit">取消</button>
                                </form>}
                              </div>
                            )}
                            {canFulfill && ["confirmed","fulfilling"].includes(order.status) && warehouses.length > 0 && <SalesFulfillmentForm lines={order.sales_order_items??[]} orderId={order.id} warehouses={warehouses}/>}<Link className="text-xs text-primary hover:underline" href={`/sales/orders/${order.id}`}>查看业务链路</Link>
                          </div>
                        </div>
                      </article>
                    );
            ...
            <section className="scroll-mt-24 overflow-hidden rounded-md border border-border/80 bg-white" id="opportunities">
              <div className="border-b border-border/70 px-5 py-5 sm:px-6">
                <h2 className="text-base font-semibold">销售机会管道</h2>
                <p className="mt-1 text-xs text-muted-foreground">从需求线索到报价、谈判与成交</p>
              </div>
              <div className="divide-y divide-border/70">
                {opportunities.length ? opportunities.map((opportunity) => (
                  <article className="grid gap-3 px-5 py-4 sm:grid-cols-[minmax(0,1fr)_120px_100px] sm:items-center sm:px-6" key={opportunity.id}>
                    <div>
                      <div className="flex flex-wrap items-center gap-2">
                        <span className="font-mono text-xs text-primary">{opportunity.opportunity_no}</span>
                        <span className="rounded-full bg-muted px-2 py-1 text-xs text-foreground">{stageLabels[opportunity.stage]}</span>
                      </div>
                      <h3 className="mt-2 text-xs font-semibold">{opportunity.title}</h3>
                      <p className="mt-1 text-xs text-muted-foreground">{relatedOne(opportunity.customers)?.name} · {opportunity.next_action || "待制定下一步"}</p>
                    </div>
                    <div>
                      <div className="text-xs text-muted-foreground">预计成交</div>
                    ...
                    <div className="text-right">
                      ...
                      <div className="mt-1 text-xs text-muted-foreground">{opportunity.probability}% 概率</div>
                    </div>
                  </article>
          ...
          {canManage && dataAvailable ? (
            <div className="space-y-5">
              <section className="rounded-md border border-border/80 bg-white p-5 sm:p-6">
                ...
                <form action={createSalesOpportunityAction} className="mt-5 space-y-3">
                  <select className="h-10 w-full rounded-md border border-border bg-white px-3 text-xs" name="customerId" required>
                    <option value="">选择客户</option>
                    {customers.map((customer) => <option key={customer.id} value={customer.id}>{customer.level}级 · {customer.name}</option>)}
                  </select>
                  <select className="h-10 w-full rounded-md border border-border bg-white px-3 text-xs" name="legalEntityId">
                  ...
                  <input className="h-10 w-full rounded-md border border-border px-3 text-xs" name="title" placeholder="机会名称 / 客户需求" required />
                  <div className="grid grid-cols-2 gap-3">
                    <input className="h-10 rounded-md border border-border px-3 text-xs" min="0" name="expectedAmountCny" placeholder="预计金额" required step="0.01" type="number" />
                    <input className="h-10 rounded-md border border-border px-3 text-xs" defaultValue="30" max="100" min="0" name="probability" placeholder="成交概率" required type="number" />
                  </div>
                  <input className="h-10 w-full rounded-md border border-border px-3 text-xs" name="expectedCloseOn" type="date" />
                  <input className="h-10 w-full rounded-md border border-border px-3 text-xs" name="source" placeholder="机会来源" />
                  <textarea className="min-h-16 w-full rounded-md border border-border px-3 py-2.5 text-xs" name="nextAction" placeholder="下一步行动" />
                  <textarea className="min-h-16 w-full rounded-md border border-border px-3 py-2.5 text-xs" name="note" placeholder="备注" />
                  <button className="h-10 w-full rounded-md bg-primary text-xs font-medium text-white" type="submit">创建销售机会</button>
                </form>
              </section>
              <section className="rounded-md border border-border/80 bg-white p-5 sm:p-6">
                <div className="mb-5 flex items-center justify-between">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">保存交易快照，确认后进入履约</p>
                  </div>
            ...
            <section className="rounded-md border border-border/80 bg-white p-6 text-center">
              <ShoppingCart className="mx-auto size-8 text-muted-foreground/50" />
              <h2 className="mt-3 text-sm font-medium">当前为销售业务查询视图</h2>
              <p className="mt-2 text-xs leading-5 text-muted-foreground">销售、客服和系统管理员可创建销售机会与订单。</p>
            </section>
          )}
        </div>
      </main>
