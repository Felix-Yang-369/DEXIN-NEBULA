      ...
      <main className="mx-auto max-w-[1440px] p-4 sm:p-6 xl:p-8">
        <section className="ui-page-header">
          ...
          <div className="relative flex flex-col justify-between gap-6 lg:flex-row lg:items-end">
            ...
            <div className="flex w-fit items-center gap-3 rounded-lg border border-white/12 bg-white/[0.07] px-4 py-3">
              <ShieldCheck className="size-5 text-muted-foreground" />
              <div>
                <div className="text-xs text-white/42">访问范围</div>
        ...
        <section className="mt-5 rounded-md border border-border/80 bg-white p-4  sm:p-5">
          <form className="flex flex-col gap-3 lg:flex-row" method="get">
            <label className="relative min-w-0 flex-1">
              ...
              <input
                aria-label="搜索制度"
                className="h-11 w-full rounded-md border border-border bg-muted pl-11 pr-4 text-xs outline-none transition-colors placeholder:text-muted-foreground/55 focus:border-primary/35 focus:bg-white focus:ring-4 focus:ring-primary/7"
            ...
            <button
              className="inline-flex h-11 items-center justify-center gap-2 rounded-md bg-primary px-5 text-xs font-medium text-primary-foreground transition-colors hover:bg-muted"
              type="submit"
            >
        ...
        {error ? (
          ...
          <section className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {filteredDocuments.map((document) => {
              ...
              return (
                <Link
                  className="group flex min-h-[285px] flex-col overflow-hidden rounded-md border border-border/80 bg-white p-6  transition duration-300  hover:border-primary/20 "
                  href={`/knowledge/${document.slug}`}
                  key={document.id}
                >
                  <div className="flex items-start justify-between gap-4">
                    <span
                      className={`rounded-full px-3 py-1.5 text-xs font-medium ${meta.tone}`}
                    >
                      {meta.label}
                    </span>
                    <span className="text-xs tracking-[0.12em] text-muted-foreground/55">
                      V{document.version}
                    </span>
                  </div>
                  <div className="mt-7 text-xs font-medium tracking-[0.14em] text-primary/55">
                  ...
                  <div className="mt-auto flex items-end justify-between gap-3 pt-6">
                    ...
                    <span className="grid size-9 place-items-center rounded-md bg-muted text-primary transition-colors group-hover:bg-primary group-hover:text-white">
        ...
        <section className="mt-5 flex flex-col gap-4 rounded-md border border-border bg-muted p-5 sm:flex-row sm:items-center sm:justify-between sm:p-6">
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="mt-1.5 max-w-3xl text-xs leading-6 text-foreground">
