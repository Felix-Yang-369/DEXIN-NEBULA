...
export default async function AccountingSettingsPage({ searchParams }: { searchParams: Promise<{ saved?: string; error?: string }> }) {
  ...
  return <WorkflowShell activeItem="财务总览" breadcrumb="财务管理 / 会计核算 / 基础设置" currentUser={{ name: employee.name, roleLabel: "会计设置" }}><main className="mx-auto max-w-[1500px] p-4 sm:p-6 xl:p-8">
    <section className="relative overflow-hidden rounded-[24px] bg-[#0a385d] px-6 py-7 text-white sm:px-8"><Cog className="absolute right-12 top-1/2 hidden size-32 -translate-y-1/2 text-white/[0.06] sm:block" /><Link className="inline-flex items-center gap-2 text-[10px] text-white/55 hover:text-white" href="/finance/accounting"><ArrowLeft className="size-3" />会计核算</Link><div className="mt-5 text-[10px] tracking-[0.15em] text-[#79d8d5]">CHART OF ACCOUNTS · FISCAL PERIOD</div><h1 className="mt-3 text-2xl font-semibold">会计基础设置</h1><p className="mt-3 text-sm text-white/55">管理科目启用状态、辅助核算要求和会计期间生命周期。</p></section>
    {!canView ? <div className="mt-5 rounded-[20px] border border-[#ead8d8] bg-white p-10 text-center text-sm text-[#965151]">暂无会计基础设置权限。</div> : <>
      {(params.error || params.saved) && <div className={`mt-5 flex items-center gap-2 rounded-xl border px-4 py-3 text-xs ${params.error ? "border-[#ead8d8] bg-[#f8eeee] text-[#965151]" : "border-[#b9dbce] bg-[#eef8f5] text-[#285f53]"}`}>{params.error ? <CircleAlert className="size-4" /> : <BadgeCheck className="size-4" />}{params.error ? errorMessages[params.error] ?? "操作失败。" : "会计基础设置已保存并记录审计。"}</div>}
      <section className="mt-5 grid gap-5 xl:grid-cols-[1.2fr_1fr]">
        <div className="rounded-[22px] border border-border bg-white p-5 sm:p-6" id="accounts"><div className="flex items-center justify-between"><div><h2 className="text-sm font-semibold">会计科目</h2><p className="mt-1 text-[10px] text-muted-foreground">{book?.name ?? "未初始化账簿"} · {accounts.length} 个科目</p></div></div>
          {canManageAccounts && book && <details className="mt-4 rounded-xl border border-[#b9dbce] bg-[#eef8f5] p-4"><summary className="flex cursor-pointer list-none items-center gap-2 text-xs font-medium text-[#285f53]"><Plus className="size-4" />新增会计科目</summary><form action={saveAccountingAccountAction} className="mt-4"><input name="bookId" type="hidden" value={book.id} /><AccountFields /><button className="mt-4 h-9 rounded-lg bg-primary px-4 text-xs text-primary-foreground" type="submit">保存科目</button></form></details>}
          <div className="mt-4 space-y-2">{accounts.map((account) => <details className="rounded-xl border border-border bg-[#fafcfe]" key={account.id}><summary className="flex cursor-pointer list-none items-center gap-3 px-4 py-3"><span className="font-mono text-xs text-primary">{account.code}</span><span className="flex-1 text-xs font-medium">{account.name}</span><span className="text-[9px] text-muted-foreground">{categoryLabels[account.category]} · {account.status === "active" ? "启用" : "停用"}</span></summary>{canManageAccounts && book && <form action={saveAccountingAccountAction} className="border-t border-border bg-white p-4"><input name="bookId" type="hidden" value={book.id} /><AccountFields account={account} /><button className="mt-4 h-9 rounded-lg border border-primary px-4 text-xs text-primary" type="submit">保存修改</button></form>}</details>)}</div>
        </div>
        <div className="rounded-[22px] border border-border bg-white p-5 sm:p-6" id="periods"><div className="flex items-center gap-2"><CalendarRange className="size-4 text-primary" /><h2 className="text-sm font-semibold">会计期间</h2></div><p className="mt-2 text-[10px] leading-5 text-muted-foreground">开放、结账和反结账均要求输入期间名称确认，并遵循时间顺序。</p>{canManagePeriods && book && <form action={createFiscalYearAction} className="mt-4 flex gap-2 rounded-xl border border-border bg-[#fafcfe] p-3"><input name="bookId" type="hidden" value={book.id} /><input className="h-9 min-w-0 flex-1 rounded-lg border border-border px-3 text-xs" defaultValue={new Date().getFullYear() + 1} max={new Date().getFullYear() + 5} min={new Date().getFullYear() - 1} name="fiscalYear" type="number" /><button className="h-9 rounded-lg border border-primary px-3 text-[9px] text-primary" type="submit">创建会计年度</button></form>}
          <div className="mt-4 space-y-2">{periods.map((period) => { const action = period.status === "future" && canManagePeriods ? "open" : period.status === "open" && canClosePeriods ? "close" : null; return <div className="rounded-xl border border-border bg-[#fafcfe] p-4" key={period.id}><div className="flex items-center justify-between"><div><div className="text-xs font-medium">{period.name}</div><div className="mt-1 text-[9px] text-muted-foreground">{period.starts_on} 至 {period.ends_on}</div></div><span className="rounded-full bg-[#edf2f7] px-2.5 py-1 text-[9px] text-[#42647a]">{statusLabels[period.status]}</span></div>{action && <form action={transitionFiscalPeriodAction} className="mt-3 flex gap-2"><input name="periodId" type="hidden" value={period.id} /><input name="action" type="hidden" value={action} /><input className="h-8 min-w-0 flex-1 rounded-lg border border-border px-2 text-[9px]" name="confirmation" placeholder={`输入“${period.name}”确认`} required /><button className="h-8 rounded-lg border border-border bg-white px-3 text-[9px] text-primary" type="submit">{action === "open" ? "开放" : "结账"}</button></form>}{period.status === "closed" && canReopen ? <Link className="mt-3 inline-flex h-8 items-center rounded-lg border border-[#ead8d8] bg-white px-3 text-[9px] text-[#965151]" href={`/finance/accounting/closing?period=${period.id}`}>前往关账工作台发起反结账</Link> : null}</div>; })}</div>
        </div>
      </section>
    </>}
  </main></WorkflowShell>;
