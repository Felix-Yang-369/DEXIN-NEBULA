...
export default async function OpeningPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  return <WorkflowShell activeItem="财务总览" breadcrumb="财务管理 / 会计核算 / 期初余额" currentUser={{ name: employee.name, roleLabel: "期初迁移" }}><main className="mx-auto max-w-[1200px] p-4 sm:p-6 xl:p-8"><section className="ui-page-header"><DatabaseZap className="absolute right-12 top-1/2 hidden size-32 -translate-y-1/2 text-white/[0.06] sm:block" /><Link className="inline-flex items-center gap-2 text-xs text-white/55 hover:text-white" href="/finance/accounting"><ArrowLeft className="size-3" />会计核算</Link><div className="mt-5 text-xs tracking-[0.15em] text-muted-foreground">OPENING BALANCE · CONTROLLED MIGRATION</div><h1 className="mt-3 text-2xl font-semibold">期初余额迁移</h1><p className="mt-3 max-w-2xl text-sm leading-7 text-white/55">仅允许在年度尚无其他凭证时生成一次期初草稿，仍需另一位财务人员审核和过账。</p></section>
    {params.error && <div className="mt-5 flex items-center gap-2 rounded-md border border-border bg-muted px-4 py-3 text-xs text-foreground"><CircleAlert className="size-4" />{errors[params.error] ?? "操作失败。"}</div>}
    {canManage !== true ? <div className="mt-5 rounded-md border border-border bg-white p-10 text-center text-sm text-foreground">暂无期初余额管理权限。</div> : <><section className="mt-5 rounded-md border border-border bg-white p-5 sm:p-6"><h2 className="text-sm font-semibold">录入期初余额</h2><p className="mt-2 text-xs leading-5 text-muted-foreground">选择开放年度首期，按科目录入借方或贷方余额。生成后不能用普通冲销流程撤销。</p>{book && eligible && accounts.length >= 2 ? <OpeningBalanceForm accounts={accounts} bookId={book.id} fiscalYear={eligible.fiscal_year} /> : <div className="mt-5 rounded-md bg-muted p-5 text-xs text-foreground">没有可迁移的开放年度，或该年度已经开始记账。</div>}</section><section className="mt-5 rounded-md border border-border bg-white p-5 sm:p-6"><h2 className="text-sm font-semibold">历史期初凭证</h2><div className="mt-4 space-y-2">{entries.map((entry) => <Link className="flex items-center justify-between rounded-md border border-border p-4 text-xs hover:border-primary/40" href={`/finance/accounting/entries/${entry.id}`} key={entry.id}><span><span className="font-mono text-primary">{entry.entry_no}</span><span className="ml-3">{entry.summary}</span></span><span className="text-muted-foreground">{entry.status}</span></Link>)}{!entries.length && <div className="rounded-md bg-muted p-6 text-center text-xs text-muted-foreground">暂无期初凭证。</div>}</div></section></>}
  </main></WorkflowShell>;
}
