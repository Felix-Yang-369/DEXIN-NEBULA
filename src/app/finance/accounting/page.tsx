  ...
  return <WorkflowShell activeItem="财务总览" breadcrumb="财务管理 / 会计核算" currentUser={{ name: employee.name, roleLabel: canView ? "财务核算" : "内部员工" }}>
    <main className="mx-auto max-w-[1440px] p-4 sm:p-6 xl:p-8">
      <section className="ui-page-header">
        ...
        <Link className="inline-flex items-center gap-2 text-xs text-white/55 hover:text-white" href="/finance"><ArrowLeft className="size-3" />财务管理中心</Link>
        <div className="mt-5 flex flex-wrap items-end justify-between gap-5"><div><div className="text-xs tracking-[0.15em] text-muted-foreground">GENERAL LEDGER · PERIOD · STATEMENT</div><h1 className="mt-3 text-2xl font-semibold">会计核算内核 V3</h1><p className="mt-3 max-w-2xl text-sm leading-7 text-white/55">在正式账簿、期间和凭证基础上，加入期初迁移、受控损益结转与三大财务报表。</p></div><div className="flex flex-wrap gap-2">{canViewStatements && <Link className="inline-flex h-9 items-center gap-2 rounded-md border border-white/15 bg-white/8 px-4 text-xs" href="/finance/accounting/statements"><BarChart3 className="size-4" />财务报表</Link>}{canViewLedger && <Link className="inline-flex h-9 items-center gap-2 rounded-md border border-white/15 bg-white/8 px-4 text-xs" href="/finance/accounting/ledger"><BookOpen className="size-4" />会计账簿</Link>}{canManageSettings && <Link className="inline-flex h-9 items-center gap-2 rounded-md border border-white/15 bg-white/8 px-4 text-xs" href="/finance/accounting/settings"><Settings2 className="size-4" />基础设置</Link>}</div></div>
      </section>
      {!canView ? <div className="mt-5 rounded-md border border-border bg-white p-10 text-center text-sm text-foreground">暂无会计核算访问权限。</div> : <>
        {(params.error || params.saved || params.transitioned) && <div className={`mt-5 flex items-center gap-2 rounded-md border px-4 py-3 text-xs ${params.error ? "border-border bg-muted text-foreground" : "border-border bg-muted text-foreground"}`}>{params.error ? <CircleAlert className="size-4" /> : <BadgeCheck className="size-4" />}{params.error ? errorMessages[params.error] ?? "操作失败。" : params.saved ? `凭证 ${params.saved} 已保存为草稿。` : params.transitioned === "review" ? "凭证已审核。" : "凭证已完成不可逆过账。"}</div>}
        <section className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          {[{ icon: Landmark, label: "当前账簿", value: currentBook?.name ?? "未初始化", note: currentBook ? `${currentBook.code} · ${currentBook.base_currency}` : "" }, { icon: CalendarRange, label: "开放期间", value: `${openPeriods.length} 个`, note: openPeriods[0]?.name ?? "暂无" }, { icon: BookOpenCheck, label: "启用科目", value: `${accounts.length} 个`, note: "企业会计准则科目基线" }, { icon: FileCheck2, label: "正式凭证", value: `${entries.length} 张`, note: `${entries.filter((entry) => entry.status === "posted").length} 张已过账` }].map((item) => { const Icon = item.icon; return <article className="rounded-md border border-border bg-white p-5" key={item.label}><Icon className="size-4 text-primary" /><div className="mt-4 text-xs text-muted-foreground">{item.label}</div><div className="mt-1 text-lg font-semibold">{item.value}</div><div className="mt-2 text-xs text-muted-foreground">{item.note}</div></article>; })}
        </section>
        {(canManageOpening || canGenerateClosing) && <section className="mt-5 grid gap-3 md:grid-cols-2">{canManageOpening && <Link className="group rounded-md border border-border bg-white p-5 hover:border-primary/40" href="/finance/accounting/opening"><DatabaseZap className="size-4 text-primary" /><div className="mt-3 text-sm font-semibold">期初余额迁移</div><p className="mt-2 text-xs leading-5 text-muted-foreground">在年度首次记账前导入已平衡期初余额。</p></Link>}{canGenerateClosing && <Link className="group rounded-md border border-border bg-white p-5 hover:border-primary/40" href="/finance/accounting/closing"><CalendarClock className="size-4 text-primary" /><div className="mt-3 text-sm font-semibold">期末损益结转</div><p className="mt-2 text-xs leading-5 text-muted-foreground">生成结转草稿并跟踪审核、过账与结账准备度。</p></Link>}</section>}
        <section className="mt-5 rounded-md border border-border bg-white p-5 sm:p-6" id="new-entry"><h2 className="text-sm font-semibold">新增会计凭证</h2><p className="mt-2 text-xs leading-5 text-muted-foreground">当前仅保存草稿；数据库会再次校验开放期间、科目归属、每行单边金额和整张凭证借贷平衡。</p>{!canCreate ? <div className="mt-5 rounded-md bg-muted p-5 text-xs text-foreground">当前账号可以查看核算数据，但没有凭证制单权限。</div> : currentBook && accounts.length >= 2 ? <JournalEntryForm accounts={accounts} bookId={currentBook.id} today={today} /> : <div className="mt-5 rounded-md bg-muted p-5 text-xs text-foreground">账簿或会计科目尚未初始化，请先执行本批数据库迁移。</div>}</section>
        <section className="mt-5 rounded-md border border-border bg-white p-5 sm:p-6" id="entries"><h2 className="text-sm font-semibold">凭证工作台</h2><div className="mt-4 overflow-x-auto rounded-md border border-border"><table className="w-full min-w-[900px] text-left text-xs"><thead className="bg-muted text-muted-foreground"><tr><th className="p-3">凭证号</th><th className="p-3">日期</th><th className="p-3">摘要</th><th className="p-3 text-right">借方</th><th className="p-3 text-right">贷方</th><th className="p-3">制单人</th><th className="p-3">状态</th><th className="p-3">操作</th></tr></thead><tbody>
          {entries.map((entry) => { const allowedTransition = entry.status === "draft" ? canReview : entry.status === "reviewed" ? canPost : false; return <tr className="border-t border-border" key={entry.id}><td className="p-3"><Link className="font-mono text-primary hover:underline" href={`/finance/accounting/entries/${entry.id}`}>{entry.entry_no}</Link></td><td className="p-3">{entry.entry_date}</td><td className="max-w-72 p-3">{entry.summary}<div className="mt-1 text-xs text-muted-foreground">{entry.source_type === "period_close_reversal" ? "反结账受控凭证" : `附件 ${entry.attachment_count}`}</div></td><td className="p-3 text-right tabular-nums">¥{Number(entry.total_debit).toFixed(2)}</td><td className="p-3 text-right tabular-nums">¥{Number(entry.total_credit).toFixed(2)}</td><td className="p-3">{relationName(entry.creator)}</td><td className="p-3"><span className={`rounded-full px-2.5 py-1 ${statusToneClass(entry.status)}`}>{statusLabels[entry.status] ?? entry.status}</span></td><td className="p-3">{allowedTransition ? <form action={transitionAccountingJournalAction}><input name="entryId" type="hidden" value={entry.id} /><input name="version" type="hidden" value={entry.version} /><input name="sourceType" type="hidden" value={entry.source_type} /><input name="action" type="hidden" value={entry.status === "draft" ? "review" : "post"} /><button className="rounded-lg border border-border px-3 py-1.5 text-xs text-primary" type="submit">{entry.status === "draft" ? "审核" : "过账"}</button></form> : <Link className="text-primary hover:underline" href={`/finance/accounting/entries/${entry.id}`}>查看明细</Link>}</td></tr>; })}
